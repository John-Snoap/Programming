...
void displayEnemies (enemy oliphants[])
{
	...
	// show all enemies that are alive
	for (int i = 1; i < (NUMBER_OF_ENEMIES + 1); i++)
	{
		if (oliphants[i].alive)
		{
			dbShowSprite (ENEMY_SPRITE_ID + i);
		} // end if
	}
...
void youWin (stats &player, DWORD color1, DWORD color2)
{
	...
	dbText ( dbScreenWidth()/2 - 110, dbScreenHeight()/2 - 10, "Press Enter to play again." );
...
void gameOver (stats &player, DWORD color1, DWORD color2)
{
	...
	dbInk (color1, color2); // The text is color1
	dbText ( dbScreenWidth()/2 - 110, dbScreenHeight()/2 - 50, "Minas Tirith is overrun." );
	dbText ( dbScreenWidth()/2 - 110, dbScreenHeight()/2 - 30, "You have failed." );
	dbText ( dbScreenWidth()/2 - 110, dbScreenHeight()/2 - 10, "Press Enter to play again." );
