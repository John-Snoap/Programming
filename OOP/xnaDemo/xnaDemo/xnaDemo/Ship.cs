...
namespace xnaDemo
{
    class Ship
    {
        ...
        public void move(Direction dir)
        {
            switch (dir)
            {
                case Direction.Left: _pos.X -= 15; // Good Guy = 7, TIE = 5, Boba Fett = 15?, Imperial SS = 2, Executor SS = 1
                    break;
                case Direction.Right: _pos.X += 15;
                    break;
                case Direction.Up: _pos.Y -= 5; // 15 seems to be a good suicide dive speed, laser speed is 10
                    break;
        ...
        public Vector2 GunPosition
        {
            get
            {
                ...
                return new Vector2(_pos.X + (Texture.Width / 2), (_pos.Y + ((Texture.Height) / 2)));
            }
        }
