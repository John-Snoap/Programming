...
public class Messenger implements Runnable {
	...
	private LL3Demon layer3Daemon;					  // reference for layer 3 daemon
	...
	public void updateObjectReferences(Factory factory){
		...
		FIB = factory.getLRPDemon().getForwardingTable(); // forwarding information base, used to populate the active names list.
		layer3Daemon = factory.getDemon3(); // layer 3 daemon, used to send messages.
	...
	private void resetSpinner(){
		validNetworkList = FIB.getList(); // get active nodes from forwarding information table.
		...
		for (RouteTableEntry entry : validNetworkList){
			if (entry.getNetworkDistancePair().getNetworkNumber().compareTo(-1) > 0 && entry.getNetworkDistancePair().getNetworkNumber().compareTo(16) < 0)  
					spinnerAdapter.add(allNamesAndNetworks.get(entry.getNetworkDistancePair().getNetworkNumber()-1)); // network numbers go from 1-15, array elements from 0-14
		}
