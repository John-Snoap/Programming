...
public class LL3Demon
{
	...
	public void receiveLL3Ppacket(byte[] packetBytes, Integer ll2pSourceAddress)
	{
		...
		if (ll3pPacket.getDestinationLL3P_IPaddressString().equalsIgnoreCase(NetworkConstants.MY_LL3P_ADDRESS))
		{
			...
			uiManager.receiveMessage(ll3pPacket.getSourceLL3P_IPaddress(), 
					ll3pPacket.getPayloadString());
		} // end if
		else
		{
	...
	public void sendLL3Ppacket(LL3P packet)
	{
		...
		else
		{
			...
			uiManager.raiseToast("Should forward message:  " + packet.getPayloadString());
		} // end else
		...
		else
		{
			...
			if (nextHopAddress != null)
			{
				...
				if (ll2pAddress != null)
				{
					...
					uiManager.raiseToast("Message:  " + packet.getPayloadString() + "  should have sent.");
				} // end if
				else
				{
					uiManager.raiseToast("ll2p for " + Integer.toHexString(nextHopAddress) + " is null.");
				} // end else
			} // end if
			else
			{
				uiManager.raiseToast("next hop for " + packet.getDestinationLL3P_IPaddressString() + " is null.");
			} // end else
		} // end else
