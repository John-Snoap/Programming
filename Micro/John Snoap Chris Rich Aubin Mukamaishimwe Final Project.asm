...
* Variables													COMMENTS
resetAddress			EQU		$E000								For the assembler
resetVector			EQU		$FFFE								For the assembler
...
* Main														COMMENTS
				ORG		resetAddress							Start program
				...
				STAA		$B000								Send sixth command -> LCD	Initialize LCD end
...
getNext				LDAA		0,X								Get next character byte
				...
				INX										Load y-coordinate
				STX		DDRB								Send y-coordinate -> PIA
				JSR		delay200us							Delay for 200 microseconds
...
ISR				CLI										Clear the interrupt flag
				...
				BRSET		0,X			%10000000	*			LCD busy check
				STAA		0,X								LCD translation -> LCD
				INC		characterCount							characterCount++
				LDAA		#5								5 -> A
				LDAB		characterCount							characterCount -> B
				CBA										if (characterCount < 5)
				BNE		done								Go to done
				CLR		characterCount							else, characterCount = 0
...
* EEPROM													COMMENTS
				...
				ORG		$FFF2								ISR vector
				FDB		ISR								ISR address
				ORG		resetVector							Reset vector
				FDB		resetAddress							Reset address
* EEPROM end 